...
function validateInput(prop, value) {
  ...
  if (prop === "email") {
    if (value.length === 0) error.email = `Please provide your aadhar no.`;
    else if (value.length !== 12)
      error.email = `Please enter a Valid aadhar-Id(12 digit long).`;
  }
  if (prop === "password") {
...
export const validate = (values) => {
  ...
  if (values.description !== undefined)
    validateInput("description", values.description);
  ...
  if (values.repassword !== undefined)
    validateInput("repassword", values.repassword);
